...
public class AuthenticationPageSteps extends FrameworkEnvironment {
    ...
    public void iEnterLogin(String login) throws Throwable {
        //ACT//
        authenticationPage.emailInput.sendKeys(login);
        logger.info(String.format("User login: %S", login));

        //ASSERT//
        Assert.assertEquals(authenticationPage.emailInput.getAttribute("value").toLowerCase(),
    ...
    public void iEnterPassword(String password) throws Throwable {
        //ACT//
        authenticationPage.passwordInput.sendKeys(password);
        logger.info(String.format("User password: %S", password));

        //ASSERT//
        Assert.assertEquals(authenticationPage.passwordInput.getAttribute("value").toLowerCase(),
    ...
    public void iCanSeeLoginForm() throws Throwable {
        Assert.assertTrue(basePage.isDisplayed(5, authenticationPage.loginForm),
    ...
    public void iCanSeeWarningMessageWithInclude(String warningMessage) throws Throwable {
        Assert.assertTrue(basePage.errorValidator(authenticationPage.loginError).toLowerCase().contains(warningMessage.toLowerCase()),
